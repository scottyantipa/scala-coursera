 ...
 */
 @RunWith(classOf[JUnitRunner])
   ...
   */
  test("one plus three is 4") {
    assert(1 + 3 === 4) // Fix me, please!
  }
  ...
  test("sum of list with repeats") {
    assert(sum(List(1,1)) === 2)
  }


  test("max throws an error for empty list") {
  ...
  test("max of a few numbers with negatives") {
    assert(max(List(-1, 2, 4)) === 4)
  }

  test("max of a few numberse with repeats") {
    assert(max(List(2,1,1,2)) === 2)
  }
